...
> **Status:** ✅ Designed (V4)
> Revision notes: adopts the ID & Correlation Standard — `orders.id` is now `UUID` (UUIDv7), generated by Order Service before the row is inserted, replacing any `BIGSERIAL`/`IDENTITY` assumption. Renames "Executor" to "Settlement Service" throughout, matching Wallet Service's naming reconciliation.
...
- Track order status: `PENDING`, `OPEN`, `PARTIALLY_FILLED`, `FILLED`, `CANCELLED`, `REJECTED`.
- Cancel eligible orders and release the remaining reserved funds.
- Publish Kafka events: `OrderCreated`, `OrderCancelRequested`, `OrderCancelled`, `OrderUpdated`.
...
Client -> Order Service -> Check ownership + status (OPEN or PARTIALLY_FILLED)
  ...
  -> Outbox Publisher polls unpublished rows -> Kafka -> Matching Engine consumes
  -> Matching Engine confirms removal -> OrderCancelled -> Order Service finalizes status + releases funds
```
...
- `GetOrder`
- `ListOrders`
- `UpdateOrderStatus` (internal, consumer-triggered only)

## Kafka Events

- `OrderCreated` — published via outbox by Order Service, consumed by Matching Engine.
- `OrderCancelRequested` — published via outbox by Order Service on the same partition key (symbol) as `OrderCreated`, consumed by Matching Engine.
- `OrderAccepted`, `OrderRejected`, `OrderPartiallyFilled`, `OrderFilled` — published by Matching Engine / Settlement Service as the order progresses.
- `OrderCancelled` — published by Matching Engine once a cancel is confirmed, consumed by Order Service.
...
orders(
  ...
  user_id UUID, market_id, side, order_type,
...
- `PENDING → OPEN` (validation passed and funds reserved)
- `PENDING → REJECTED` (validation failed or funds could not be reserved)
- `OPEN → PARTIALLY_FILLED` (partial match)
- `OPEN → FILLED` (fully matched immediately)
- `PARTIALLY_FILLED → FILLED` (remaining quantity matched)
- `OPEN → CANCELLED` / `PARTIALLY_FILLED → CANCELLED`

## Failure Handling

- Idempotency key prevents duplicate orders on client retry.
- On `REJECTED`: release the full reserved amount (nothing was ever matched).
...
|---|---|---|
| Validation fails | Order never reserved funds | Reject immediately, no compensation needed |
| Fund reservation fails | Wallet Service declines (insufficient balance) | Reject order, status `REJECTED`, no funds were locked |
...
- Support only Good Till Cancelled (GTC).
- Limit orders remain `OPEN` until filled or cancelled; reserved funds stay locked while `OPEN`.
- IOC, FOK, and GTD are deferred to a later version.
